...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/dim-reduction/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/10-dim-reduction.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/dim-reduction"
TOTAL_FASES=5
...
count_pending_checkboxes() {
    local file="$1"; local n
    n=$(grep -c '\- \[ \]' "$file" 2>/dev/null) || n=0; echo "$n"
}

count_completed_checkboxes() {
    local file="$1"; local n
    n=$(grep -c '\- \[x\]' "$file" 2>/dev/null) || n=0; echo "$n"
}
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em machine learning, reducao de dimensionalidade e model risk management.
...
9. Para dim reduction: use MNIST digits (sklearn), Swiss roll e Iris como datasets principais

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/dim-reduction/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/dim-reduction/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/dim-reduction/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/dim-reduction"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/dim-reduction/modeling"
    "$MRL_ROOT/content/en/code-examples/dim-reduction/validation"
    "$MRL_ROOT/content/en/code-examples/dim-reduction/visualization"
    "$MRL_ROOT/content/en/report-templates/dim-reduction"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Dimensionality Reduction Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Dimensionality Reduction"
log_separator
...
ls "$MRL_ROOT/content/en/guides/dim-reduction/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "pca\|tsne\|umap\|autoencoder\|dimensionality\|dim-reduction\|curse" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/dim-reduction/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/dim-reduction/" 2>/dev/null || echo "    (nenhum)"
