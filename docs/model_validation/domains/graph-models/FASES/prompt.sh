...
# ============================================================
# CONTENT GENERATION — Graph Models Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/graph-models/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/23-graph-models.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/graph-models"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em graph neural networks, knowledge graphs e model risk management.
...
8. Stack: torch-geometric + pykeen + networkx, Cora (PyG) como dataset principal

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/graph-models/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/graph-models/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/graph-models/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/graph-models" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/graph-models/modeling" "$MRL_ROOT/content/en/code-examples/graph-models/validation" "$MRL_ROOT/content/en/code-examples/graph-models/visualization" "$MRL_ROOT/content/en/report-templates/graph-models" "$MRL_ROOT/content/pt/guides/graph-models" "$MRL_ROOT/content/pt/code-examples/graph-models/modeling" "$MRL_ROOT/content/pt/code-examples/graph-models/validation" "$MRL_ROOT/content/pt/code-examples/graph-models/visualization" "$MRL_ROOT/content/pt/report-templates/graph-models")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — Graph Models Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: Graph Models"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/graph-models/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "graph\|gnn\|gcn\|graphsage\|knowledge.*graph\|over-smoothing\|link-prediction\|fraud.*detection" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/graph-models/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/graph-models/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
