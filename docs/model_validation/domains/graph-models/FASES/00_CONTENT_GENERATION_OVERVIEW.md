...
Gerar todo o conteudo do dominio "Graph Models" para o ModelRiskLab.

## Referencia

- Especificacao do dominio: `/home/guhaase/projetos/Graphenda/build/docs/model_validation/domains/23-graph-models.md`
...
| PENDENTE | CONTENT_GEN_F4.md     | Codigo: GCN, GraphSAGE, KG embeddings, Node2Vec, eval, over-smoothing, plots | F3           |
...
│   └── pitfalls.md            ← F3
├── news/                      ← F2
├── code-examples/graph-models/
│   ├── modeling/              ← F4
│   │   ├── gcn_node_classification.py
│   │   ├── graphsage_inductive.py
│   │   ├── knowledge_graph_embeddings.py
│   │   └── node2vec_embeddings.py
│   ├── validation/            ← F4
│   │   ├── graph_model_eval.py
│   │   └── over_smoothing_analysis.py
│   └── visualization/         ← F4
│       └── graph_plots.py
└── report-templates/graph-models/
    ├── template.md            ← F5
    └── example.md             ← F5
```
...
- [ ] 7 arquivos de codigo (GCN, GraphSAGE, KG embeddings, Node2Vec, graph eval, over-smoothing, graph plots)
- [ ] Template de relatorio + exemplo com GCN fraud detection em transaction network
