...
# ============================================================
# CONTENT GENERATION — Clustering Models Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/clustering/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/09-clustering.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/clustering"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em machine learning, clustering e model risk management.
...
9. Para clustering: use Iris (sklearn), Mall Customers e sinteticos como datasets principais

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/clustering/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/clustering/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/clustering/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/clustering"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/clustering/modeling"
    "$MRL_ROOT/content/en/code-examples/clustering/validation"
    "$MRL_ROOT/content/en/code-examples/clustering/visualization"
    "$MRL_ROOT/content/en/report-templates/clustering"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Clustering Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Clustering Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/clustering/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "k-means\|dbscan\|cluster\|silhouette\|gmm\|segmentation" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/clustering/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/clustering/" 2>/dev/null || echo "    (nenhum)"
