...
# ============================================================
# CONTENT GENERATION — NLP Embeddings Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/nlp-embeddings/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/17-nlp-embeddings.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/nlp-embeddings"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em NLP, word/sentence embeddings, BERT e model risk management.
...
8. Stack: transformers + sentence-transformers + gensim

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/nlp-embeddings/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/nlp-embeddings/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/nlp-embeddings/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/nlp-embeddings" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/nlp-embeddings/modeling" "$MRL_ROOT/content/en/code-examples/nlp-embeddings/validation" "$MRL_ROOT/content/en/code-examples/nlp-embeddings/visualization" "$MRL_ROOT/content/en/report-templates/nlp-embeddings")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — NLP Embeddings Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: NLP Embeddings"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/nlp-embeddings/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "word2vec\|bert.*explained\|fine-tun.*bert\|bias.*embedding\|sentence.*embedding\|pretrained\|embedding.*quality" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/nlp-embeddings/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/nlp-embeddings/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
