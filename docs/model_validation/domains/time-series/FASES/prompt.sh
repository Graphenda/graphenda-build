...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/time-series/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/03-time-series.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/time-series"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em econometria, series temporais e model risk management.
...
9. Para series temporais: use AirPassengers (statsmodels) e retornos financeiros como datasets principais

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/time-series/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/time-series/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/time-series/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/time-series"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/time-series/modeling"
    "$MRL_ROOT/content/en/code-examples/time-series/validation"
    "$MRL_ROOT/content/en/code-examples/time-series/visualization"
    "$MRL_ROOT/content/en/report-templates/time-series"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Time Series Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Time Series Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/time-series/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "arima\|garch\|stationarity\|var-model\|forecast\|time-series\|cointegration" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/time-series/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/time-series/" 2>/dev/null || echo "    (nenhum)"
