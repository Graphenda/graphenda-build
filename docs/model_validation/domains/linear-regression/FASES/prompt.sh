...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/linear-regression/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/01-linear-regression.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/linear-regression"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em model risk management.
...
$(if [ "$fase_num" -gt 1 ]; then
    ...
    echo "- Guides: $MRL_ROOT/content/en/guides/linear-regression/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/linear-regression/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/linear-regression/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/linear-regression"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/linear-regression/modeling"
    "$MRL_ROOT/content/en/code-examples/linear-regression/validation"
    "$MRL_ROOT/content/en/code-examples/linear-regression/visualization"
    "$MRL_ROOT/content/en/report-templates/linear-regression"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Linear Regression Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Linear Regression"
log_separator
...
ls "$MRL_ROOT/content/en/guides/linear-regression/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep "linear-regression\|regression\|ols\|lasso\|ridge\|r-squared\|multicollinearity\|outlier" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/linear-regression/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/linear-regression/" 2>/dev/null || echo "    (nenhum)"
