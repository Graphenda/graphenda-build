...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/deep-learning-tabular/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/14-deep-learning-tabular.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/deep-learning-tabular"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em deep learning para dados tabulares e model risk management.
...
9. Para DL tabular: use PyTorch + pytorch-tabnet, Adult Income (UCI) como dataset principal

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/deep-learning-tabular/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/deep-learning-tabular/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/deep-learning-tabular/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/deep-learning-tabular" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/deep-learning-tabular/modeling" "$MRL_ROOT/content/en/code-examples/deep-learning-tabular/validation" "$MRL_ROOT/content/en/code-examples/deep-learning-tabular/visualization" "$MRL_ROOT/content/en/report-templates/deep-learning-tabular")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — Deep Learning Tabular Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: Deep Learning Tabular"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/deep-learning-tabular/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "tabnet\|ft-transformer\|deep-learning.*tabular\|gradient-boosting.*tabular\|embedding.*categorical\|banks.*xgboost\|regulated" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/deep-learning-tabular/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/deep-learning-tabular/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
