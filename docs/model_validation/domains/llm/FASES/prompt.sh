...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/llm/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/18-llm.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/llm"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em Large Language Models, RAG, RLHF e model risk management.
...
8. Stack: transformers + langchain + PEFT + RAGAS

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/llm/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/llm/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/llm/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/llm" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/llm/modeling" "$MRL_ROOT/content/en/code-examples/llm/validation" "$MRL_ROOT/content/en/code-examples/llm/visualization" "$MRL_ROOT/content/en/report-templates/llm")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — Large Language Models Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: Large Language Models"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/llm/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "llm\|rag.*fine\|hallucination\|eu-ai-act\|red-team\|mmlu\|production-rag" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/llm/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/llm/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
