...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/ensemble/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/08-ensemble.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/ensemble"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em machine learning, metodos ensemble e model risk management.
...
9. Para ensembles: use Breast Cancer (sklearn) e German Credit como datasets principais

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/ensemble/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/ensemble/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/ensemble/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/ensemble"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/ensemble/modeling"
    "$MRL_ROOT/content/en/code-examples/ensemble/validation"
    "$MRL_ROOT/content/en/code-examples/ensemble/visualization"
    "$MRL_ROOT/content/en/report-templates/ensemble"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Ensemble Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Ensemble Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/ensemble/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "bagging\|boosting\|stacking\|ensemble\|out-of-bag\|blending\|diversity" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/ensemble/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/ensemble/" 2>/dev/null || echo "    (nenhum)"
