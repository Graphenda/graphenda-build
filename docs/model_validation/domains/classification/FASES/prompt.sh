...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/classification/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/06-classification.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/classification"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em machine learning, classificacao e model risk management.
...
9. Para classificacao: use Breast Cancer (sklearn) e Credit Card Fraud como datasets principais

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/classification/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/classification/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/classification/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/classification"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/classification/modeling"
    "$MRL_ROOT/content/en/code-examples/classification/validation"
    "$MRL_ROOT/content/en/code-examples/classification/visualization"
    "$MRL_ROOT/content/en/report-templates/classification"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Classification Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Classification Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/classification/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "random-forest\|xgboost\|auc-roc\|shap\|imbalanced\|fairness\|hyperparameter\|classification" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/classification/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/classification/" 2>/dev/null || echo "    (nenhum)"
