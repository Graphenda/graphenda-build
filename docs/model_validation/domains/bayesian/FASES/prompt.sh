...
# ============================================================
# CONTENT GENERATION — Bayesian Models Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/bayesian/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/11-bayesian.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/bayesian"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em estatistica bayesiana, programacao probabilistica e model risk management.
...
9. Para bayesiano: use PyMC + ArviZ como stack principal, Radon e Eight Schools como datasets

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/bayesian/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/bayesian/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/bayesian/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/bayesian"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/bayesian/modeling"
    "$MRL_ROOT/content/en/code-examples/bayesian/validation"
    "$MRL_ROOT/content/en/code-examples/bayesian/visualization"
    "$MRL_ROOT/content/en/report-templates/bayesian"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Bayesian Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Bayesian Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/bayesian/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "bayesian\|mcmc\|gaussian-process\|hierarchical\|loo-cv\|waic\|prior" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/bayesian/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/bayesian/" 2>/dev/null || echo "    (nenhum)"
