...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/generative/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/19-generative.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/generative"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em modelos generativos, GANs, dados sinteticos e model risk management.
...
8. Stack: sdv + torch + sdmetrics, Adult Income (UCI) como dataset principal

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/generative/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/generative/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/generative/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/generative" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/generative/modeling" "$MRL_ROOT/content/en/code-examples/generative/validation" "$MRL_ROOT/content/en/code-examples/generative/visualization" "$MRL_ROOT/content/en/report-templates/generative")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — Generative Models Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: Generative Models"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/generative/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "gan\|synthetic.*data\|fid.*score\|mode.*collapse\|diffusion.*frontier\|validating.*synthetic\|regulatory.*synthetic" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/generative/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/generative/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
