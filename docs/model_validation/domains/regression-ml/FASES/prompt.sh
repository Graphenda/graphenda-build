...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/regression-ml/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/07-regression-ml.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/regression-ml"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em machine learning, regressao e model risk management.
...
9. Para regressao ML: use California Housing (sklearn) e Ames Housing como datasets principais

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/regression-ml/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/regression-ml/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/regression-ml/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/regression-ml"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/regression-ml/modeling"
    "$MRL_ROOT/content/en/code-examples/regression-ml/validation"
    "$MRL_ROOT/content/en/code-examples/regression-ml/visualization"
    "$MRL_ROOT/content/en/report-templates/regression-ml"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Regression ML Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Regression ML Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/regression-ml/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "tree-based\|svr\|feature-importance\|residual\|quantile\|overfitting.*tree\|real-estate\|regression-ml" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/regression-ml/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/regression-ml/" 2>/dev/null || echo "    (nenhum)"
