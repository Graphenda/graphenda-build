...
# ============================================================
# CONTENT GENERATION — Deep Learning Sequential Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/deep-learning-sequential/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/15-deep-learning-sequential.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/deep-learning-sequential"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em deep learning sequencial, series temporais e model risk management.
...
9. Para DL sequencial: use PyTorch, ETTh1/AirPassengers como datasets, walk-forward validation

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/deep-learning-sequential/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/deep-learning-sequential/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/deep-learning-sequential/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/deep-learning-sequential" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/deep-learning-sequential/modeling" "$MRL_ROOT/content/en/code-examples/deep-learning-sequential/validation" "$MRL_ROOT/content/en/code-examples/deep-learning-sequential/visualization" "$MRL_ROOT/content/en/report-templates/deep-learning-sequential")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — Deep Learning Sequential Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: Deep Learning Sequential"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/deep-learning-sequential/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "lstm\|transformer.*time\|gru\|tft\|sequential\|simple.*beat\|attention.*interpret\|financial.*time" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/deep-learning-sequential/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/deep-learning-sequential/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
