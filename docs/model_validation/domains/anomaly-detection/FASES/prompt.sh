...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/anomaly-detection/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/21-anomaly-detection.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/anomaly-detection"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em anomaly detection, fraud detection e model risk management.
...
- Dataset principal: Credit Card Fraud (Kaggle) — 284,807 transacoes, 492 fraudes (0.172%)
...
8. Stack: sklearn + pyod + imbalanced-learn + xgboost + torch, Credit Card Fraud (Kaggle) como dataset principal

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/anomaly-detection/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/anomaly-detection/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/anomaly-detection/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/anomaly-detection" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/anomaly-detection/modeling" "$MRL_ROOT/content/en/code-examples/anomaly-detection/validation" "$MRL_ROOT/content/en/code-examples/anomaly-detection/visualization" "$MRL_ROOT/content/en/report-templates/anomaly-detection")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — Anomaly Detection Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: Anomaly Detection"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/anomaly-detection/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "isolation-forest\|fraud.*paypal\|auc-pr\|alert-fatigue\|cost-sensitive\|aml-regulation\|temporal-validation.*fraud" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/anomaly-detection/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/anomaly-detection/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
