...
# ============================================================
# CONTENT GENERATION — Survival Models Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/survival/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/05-survival.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/survival"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em analise de sobrevivencia, bioestatistica e model risk management.
...
9. Para sobrevivencia: use Lung Cancer e Rossi Recidivism (lifelines) como datasets principais

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/survival/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/survival/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/survival/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/survival"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/survival/modeling"
    "$MRL_ROOT/content/en/code-examples/survival/validation"
    "$MRL_ROOT/content/en/code-examples/survival/visualization"
    "$MRL_ROOT/content/en/report-templates/survival"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Survival Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Survival Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/survival/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "survival\|cox\|kaplan\|competing\|hazard\|c-statistic" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/survival/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/survival/" 2>/dev/null || echo "    (nenhum)"
