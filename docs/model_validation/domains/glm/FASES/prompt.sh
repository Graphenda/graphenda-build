...
# ============================================================
# CONTENT GENERATION — Generalized Linear Models Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/glm/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/04-glm.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/glm"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em estatistica, GLMs e model risk management.
...
9. Para GLMs: use German Credit Data (credit scoring) e datasets de contagem como referencia

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/glm/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/glm/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/glm/"
fi)
...
DEST_DIRS=(
    "$MRL_ROOT/content/en/guides/glm"
    "$MRL_ROOT/content/en/news"
    "$MRL_ROOT/content/en/code-examples/glm/modeling"
    "$MRL_ROOT/content/en/code-examples/glm/validation"
    "$MRL_ROOT/content/en/code-examples/glm/visualization"
    "$MRL_ROOT/content/en/report-templates/glm"
)
...
# --- INICIO DA EXECUCAO ---
log_separator
log "CONTENT GENERATION — Generalized Linear Models Domain"
...
# --- RESUMO ---
log_separator
log "RESUMO — CONTENT GENERATION: Generalized Linear Models"
log_separator
...
ls "$MRL_ROOT/content/en/guides/glm/" 2>/dev/null || echo "    (nenhum)"

echo -e "${YELLOW}  News:${NC}"
ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "logistic\|poisson\|link-function\|hosmer\|overdispersion\|glm\|credit-scoring" || echo "    (nenhum novo)"

echo -e "${YELLOW}  Code Examples:${NC}"
find "$MRL_ROOT/content/en/code-examples/glm/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
[ $? -ne 0 ] && echo "    (nenhum)"

echo -e "${YELLOW}  Report Templates:${NC}"
ls "$MRL_ROOT/content/en/report-templates/glm/" 2>/dev/null || echo "    (nenhum)"
