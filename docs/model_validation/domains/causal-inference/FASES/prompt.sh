...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/causal-inference/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/22-causal-inference.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/causal-inference"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em inferencia causal, econometria e model risk management.
...
8. Stack: econml + dowhy + statsmodels, LaLonde NSW como dataset principal

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/causal-inference/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/causal-inference/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/causal-inference/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/causal-inference" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/causal-inference/modeling" "$MRL_ROOT/content/en/code-examples/causal-inference/validation" "$MRL_ROOT/content/en/code-examples/causal-inference/visualization" "$MRL_ROOT/content/en/report-templates/causal-inference" "$MRL_ROOT/content/pt/guides/causal-inference" "$MRL_ROOT/content/pt/code-examples/causal-inference/modeling" "$MRL_ROOT/content/pt/code-examples/causal-inference/validation" "$MRL_ROOT/content/pt/code-examples/causal-inference/visualization" "$MRL_ROOT/content/pt/report-templates/causal-inference")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — Causal Inference Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: Causal Inference"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/causal-inference/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "causal\|did\|dag\|propensity\|sensitivity\|ab-test.*causal\|causal-forest" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/causal-inference/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/causal-inference/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
