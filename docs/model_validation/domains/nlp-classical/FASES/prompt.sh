...
# ============================================================
# CONTENT GENERATION — NLP Classical Domain
...
SPEC_DIR="$PROJECT_ROOT/build/docs/model_validation/domains/nlp-classical/FASES"
DOMAIN_SPEC="$PROJECT_ROOT/build/docs/model_validation/domains/16-nlp-classical.md"
LOG_DIR="$PROJECT_ROOT/logs/content-generation/nlp-classical"
TOTAL_FASES=5
...
build_prompt() {
    ...
    cat <<PROMPT
Voce e um pesquisador e redator tecnico especializado em NLP classico, text mining e model risk management.
...
9. Para NLP classico: use sklearn + gensim + nltk, 20 Newsgroups como dataset principal

## Conteudo ja gerado em fases anteriores
$(if [ "$fase_num" -gt 1 ]; then
    echo "Verifique o conteudo ja gerado em:"
    echo "- Guides: $MRL_ROOT/content/en/guides/nlp-classical/"
    echo "- News: $MRL_ROOT/content/en/news/"
    echo "- Code: $MRL_ROOT/content/en/code-examples/nlp-classical/"
    echo "- Reports: $MRL_ROOT/content/en/report-templates/nlp-classical/"
fi)
...
DEST_DIRS=("$MRL_ROOT/content/en/guides/nlp-classical" "$MRL_ROOT/content/en/news" "$MRL_ROOT/content/en/code-examples/nlp-classical/modeling" "$MRL_ROOT/content/en/code-examples/nlp-classical/validation" "$MRL_ROOT/content/en/code-examples/nlp-classical/visualization" "$MRL_ROOT/content/en/report-templates/nlp-classical")
for dir in "${DEST_DIRS[@]}"; do [ ! -d "$dir" ] && mkdir -p "$dir"; done
log_ok "Diretorios de destino verificados"

log_separator
log "CONTENT GENERATION — NLP Classical Domain"
...
log_separator; log "RESUMO — CONTENT GENERATION: NLP Classical"; log_separator
for i in $(seq 1 $TOTAL_FASES); do
    SPEC_FILE="$SPEC_DIR/CONTENT_GEN_F${i}.md"
...
echo -e "${YELLOW}  Guides:${NC}"; ls "$MRL_ROOT/content/en/guides/nlp-classical/" 2>/dev/null || echo "    (nenhum)"
echo -e "${YELLOW}  News:${NC}"; ls "$MRL_ROOT/content/en/news/" 2>/dev/null | grep -i "tfidf\|lda\|ner.*traditional\|sentiment.*simple\|preprocessing\|classical.*nlp\|text.*classification.*production" || echo "    (nenhum novo)"
echo -e "${YELLOW}  Code Examples:${NC}"; find "$MRL_ROOT/content/en/code-examples/nlp-classical/" -name "*.py" 2>/dev/null | while read f; do echo "    $(echo $f | sed "s|$MRL_ROOT/content/en/code-examples/||")"; done
echo -e "${YELLOW}  Report Templates:${NC}"; ls "$MRL_ROOT/content/en/report-templates/nlp-classical/" 2>/dev/null || echo "    (nenhum)"
echo ""; log_separator
